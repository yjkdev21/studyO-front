...
export default function GroupDetail() {
  ...
  const getThumbnailUrl = (group) => {
    ...
    if (
      group.thumbnailFullPath &&
      !group.thumbnailFullPath.includes("default")
    ) {
  ...
  const handleDelete = async () => {
    if (!canDelete()) {
      if (!isOwner()) {
        ...
        alert(
          "다른 멤버가 있어 삭제할 수 없습니다. 모든 멤버가 나간 후 삭제해주세요."
        );
      }
      return;
    }
    ...
    const fetchGroup = async () => {
      ...
      try {
        ...
        if (response.data && response.data.data) {
          ...
          // S3 썸네일 URL 검증
          if (
            groupData.thumbnailFullPath &&
            !groupData.thumbnailFullPath.includes("default")
          ) {
        ...
        try {
          const memberResponse = await axios.get(
            `${host}/api/study/${groupId}/members`,
            { withCredentials: true }
          );
    ...
    <div id="groupdetail" className="container mt-4">
      <div className="view-header-section">
        ...
        <div className="view-author-info">
          <img
            src={
              group.ownerProfileImageFullPath || "/images/default-profile.png"
            }
            alt="프로필 이미지"
            style={{
              width: "40px",
